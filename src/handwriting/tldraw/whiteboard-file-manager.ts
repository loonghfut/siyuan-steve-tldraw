    ...
    private static extractDrawingId(fileName: string): string {
        ...
        if (tsMatch) {
            ...
            if (lastDash > prefix.length) {
                storageKey = withoutTs.slice(0, lastDash); // 得到 "tldraw-data-{id}"
            }
            if (storageKey.startsWith(prefix)) {
                const id = storageKey.slice(prefix.length);
                // 如果 id 本身包含我们期望的严格格式，优先返回严格匹配部分
